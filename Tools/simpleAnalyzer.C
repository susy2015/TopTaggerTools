...
int main(int argc, char* argv[])
{
    ...
    bool runOnCondor = false, enableTTbar = false, doWgt = true, enableStored = false, overrideStored = false, noCorr = false;
    int nFiles = -1, startFile = 0, nEvts = -1;
    std::string dataSets = "Signal_T2tt_mStop850_mLSP100", filename = "example.root";

    int JECSys = 0;

    while((opt = getopt_long(argc, argv, "ctsradubD:N:M:E:O:", long_options, &option_index)) != -1)
    {
        switch(opt)
        {
        case 'c':
    ...
    int events = 0, eventsPhoton = 0, eventsQCD = 0, eventsLowHTQCD = 0, eventsTaggedLowHTQCD = 0, events1L = 0, events2L = 0, eventsTTbar1l = 0, eventsTTbar2l = 0, eventsTTbarNol = 0;
...
//                return 0;
                ...
                //const double& highestDisc = tr.getVar<double>("highestDisc");
